...
fn engine_open_database_creates_and_migrates() {
    use dspatch_engine::engine::startup::open_database;
...
async fn client_api_server_starts_and_responds_to_health() {
    ...
    let server_handle = tokio::spawn(async move {
        ...
        let app = dspatch_engine::client_api::server::build_router(runtime_clone.clone());
...
fn protocol_command_frame_serialization() {
    use dspatch_engine::client_api::protocol::ClientFrame;
...
fn protocol_result_frame_serialization() {
    use dspatch_engine::client_api::protocol::ServerFrame;
...
fn service_registry_provides_all_services() {
    ...
    let tmp = tempfile::tempdir().unwrap();
    let db_path = tmp.path().join("test.db");
    let db = dspatch_engine::engine::startup::open_database(&db_path).unwrap();
...
fn error_mapping_covers_all_app_error_variants() {
    use dspatch_engine::client_api::error_mapping::error_to_code;
...
async fn dispatch_get_workspace_not_found() {
    ...
    use dspatch_engine::client_api::dispatch::dispatch_command;
    use dspatch_engine::engine::service_registry::ServiceRegistry;
    ...
    assert!(matches!(err, dspatch_engine::util::error::AppError::NotFound(_)));
}
...
fn error_to_server_frame_preserves_message_and_id() {
    use dspatch_engine::client_api::error_mapping::error_to_frame;
    use dspatch_engine::client_api::protocol::ServerFrame;
    use dspatch_engine::util::error::AppError;
...
async fn ephemeral_event_emitter_broadcasts_events() {
    use dspatch_engine::client_api::ephemeral::EphemeralEventEmitter;
