...
use dspatch_engine::db::dao::{AgentTemplateDao, ApiKeyDao, PreferenceDao, WorkspaceDao};
use dspatch_engine::db::key_manager::testing::InMemorySecretStore;
use dspatch_engine::db::Database;
use dspatch_engine::domain::enums::SourceType;
use dspatch_engine::domain::models::CreateAgentTemplateRequest;
use dspatch_engine::sdk::DspatchSdk;
use dspatch_engine::services::{
...
async fn cli_init_creates_config_file() {
    ...
    dspatch_engine::cli::commands::init::run(Some(&dir_str))
        .await
        .unwrap();
...
async fn cli_init_fails_if_file_exists() {
    ...
    let result = dspatch_engine::cli::commands::init::run(Some(&dir_str)).await;
...
async fn cli_validate_on_init_output() {
    ...
    let result = dspatch_engine::cli::commands::validate::run(Some(&dir_str)).await;
    assert!(result.is_ok());
}
...
async fn cli_init_then_validate_end_to_end() {
    ...
    // Validate.
    dspatch_engine::cli::commands::validate::run(Some(&dir_str))
        .await
        .unwrap();

    // Parse with the workspace config parser directly.
    let config =
        dspatch_engine::workspace_config::parser::parse_workspace_config_file(dir.path()).unwrap();
...
fn workspace_config_parse_validate_encode_roundtrip() {
    use dspatch_engine::workspace_config::parser::{encode_yaml, parse_workspace_config};
    use dspatch_engine::workspace_config::validation::validate_config;
...
fn signal_session_establish_and_encrypt() {
    use dspatch_engine::signal::protocol::{PreKeyBundle, SignalManager};
...
fn sync_engine_cross_engine_outbox_apply() {
    use dspatch_engine::sync::message::SyncOp;
    use dspatch_engine::sync::peer_connection::PeerConnectionManager;
    use dspatch_engine::sync::sync_engine::SyncEngine;
    ...
    let signal_a = Arc::new(tokio::sync::Mutex::new(
        dspatch_engine::signal::SignalManager::new(Arc::clone(&db_a), 1, signing_key_a),
    ));
    let signal_b = Arc::new(tokio::sync::Mutex::new(
        dspatch_engine::signal::SignalManager::new(Arc::clone(&db_b), 2, signing_key_b),
    ));
...
async fn workspace_create_and_verify() {
    use dspatch_engine::domain::models::CreateWorkspaceRequest;
...
fn workspace_config_flatten_agent_hierarchy() {
    use dspatch_engine::workspace_config::flat_agent::flatten_agent_hierarchy;
    use dspatch_engine::workspace_config::parser::parse_workspace_config;
