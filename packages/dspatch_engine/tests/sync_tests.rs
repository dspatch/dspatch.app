...
use dspatch_engine::sync::message::{SyncChange, SyncMessage, SyncOp};
...
fn test_engine(device_id: &str) -> SyncEngine {
    ...
    let signal_svc = dspatch_engine::signal::SignalService::new(
        db.conn_arc().clone(),
        1,
        identity_key_pair,
    );
...
async fn test_engine_pair() -> (SyncEngine, SyncEngine) {
    ...
    let signal_a = Arc::new(Mutex::new(dspatch_engine::signal::SignalService::new(
        db_a.conn_arc().clone(), 1, identity_a,
    )));
    let signal_b = Arc::new(Mutex::new(dspatch_engine::signal::SignalService::new(
