...
use dspatch_engine::workspace_config::config::*;
use dspatch_engine::workspace_config::env_resolver;
use dspatch_engine::workspace_config::flat_agent::{self, flatten_agent_hierarchy};
use dspatch_engine::workspace_config::parser::{encode_yaml, parse_workspace_config};
use dspatch_engine::workspace_config::path_resolver;
...
fn parse_and_write_file_roundtrip() {
    ...
    dspatch_engine::workspace_config::parser::write_workspace_config(tmp.path(), &config).unwrap();

    let reparsed =
        dspatch_engine::workspace_config::parser::parse_workspace_config_file(tmp.path()).unwrap();
    assert_eq!(config, reparsed);
}
...
fn parse_file_not_found() {
    let tmp = tempfile::tempdir().unwrap();
    let result =
        dspatch_engine::workspace_config::parser::parse_workspace_config_file(tmp.path());
    assert!(result.is_err());
}
