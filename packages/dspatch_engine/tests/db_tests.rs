...
use dspatch_engine::db::health::{open_checked, DbHealthStatus};
use dspatch_engine::db::key_manager::DatabaseKeyManager;
use dspatch_engine::db::migrations::SCHEMA_VERSION;
use dspatch_engine::db::reactive::{watch_query, TableChangeTracker};
use dspatch_engine::db::schema::TABLE_NAMES;
...
fn test_schema_creation_all_15_tables() {
    ...
    assert_eq!(tables.len(), 28, "Expected 28 tables, got: {tables:?}");
...
fn test_migrations_run_without_error() {
    ...
    use dspatch_engine::db::migrations::run_migrations;
...
fn migration_v11_creates_ephemeral_state_tables() {
    ...
    dspatch_engine::db::migrations::create_tables(&conn).unwrap();
    conn.execute_batch(
