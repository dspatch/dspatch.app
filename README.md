...
- **Cross-platform** — Windows and macOS today (from source). Linux, iOS, Android and pre-built binaries coming soon
...
> **Pre-built binaries are coming soon.** For now, d:spatch must be built from source. It works on **Windows**, **macOS**, and probably **Linux**.
...
**Linux only:** you also need system libraries for the GTK build:
...
# Run code generation (Freezed, Riverpod, etc.)
...
> **Note:** The app depends on a Rust SDK via [flutter_rust_bridge](https://github.com/aspect-build/rules_swc). The Rust code is compiled automatically as part of the Flutter build process — no separate Rust build step is needed.
