...
class AgentSourceScanner {
  ...
  /// Checks whether [directory] is the root of a git repository.
  static bool isGitRepository(String directory) {
    return Directory('$directory/.git').existsSync();
  }

  /// Returns the `origin` remote URL for the git repo at [directory].
  static Future<String?> getGitRemoteUrl(String directory) async {
    try {
  ...
  /// Returns the current branch name for the git repo at [directory].
  static Future<String?> getGitBranch(String directory) async {
    try {
      final result = await Process.run(
      ...
      return (branch.isNotEmpty && branch != 'HEAD') ? branch : null;
    } catch (_) {
      return null;
    }
  }

  /// Returns `true` if the git repo at [directory] has uncommitted changes.
  static Future<bool> hasUncommittedChanges(String directory) async {
    try {
      final result = await Process.run(
  ...
  /// Returns `true` if the git repo at [directory] has unpushed commits.
  static Future<bool> hasUnpushedCommits(
    String directory,
    String branch,
  ) async {
    try {
  ...
  static String sshToHttpsUrl(String url) {
    final sshPattern = RegExp(r'^([\w.-]+)@([\w.-]+):(.+)$');
    final match = sshPattern.firstMatch(url);
